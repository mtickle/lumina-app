...
const DeepDiveDrawer = ({ isOpen, onClose, activeCard }) => {
  ...
  useEffect(() => {
    const fetchDeepDive = async () => {
      ...
      if (
        activeCard.card_type === "PLACE" &&
        !activeCard.payload?.hasDeepDive
      ) {
        setContent(
          activeCard.payload.description || "Description unavailable."
        );
        return;
      }
  ...
  return (
    <div
      className={`absolute inset-0 z-50 transition-opacity duration-300 ${isOpen
          ...
          <div className="flex w-full justify-between items-start gap-4">
            <div className="flex flex-col">
              <h2 className="text-white font-bold text-2xl leading-tight">
                {activeCard?.payload?.title || "Deep Dive"}
              </h2>
              {/* Optional Date Context Sub-header */}
              ...
              <div className="max-w-none">
                <ReactMarkdown
                  components={{
                    ...
                    p: ({ node, ...props }) => <p className="text-zinc-300 text-lg leading-relaxed mb-5" {...props} />,
                    ul: ({ node, ...props }) => <ul className="list-disc list-outside ml-5 mb-6 text-zinc-300 text-lg space-y-2" {...props} />,
